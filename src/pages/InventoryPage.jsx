...
function InventoryPage() {
  ...
  const estRestockCost = allProducts
    ...
    .reduce(
      (acc, p) =>
        acc +
        Math.max(0, Number(p.minimum_stock || 5) - Number(p.quantity || 0)) *
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <section className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* Right Actions: Refresh, Action Button */}
        <div className="flex flex-wrap items-center gap-2">
      ...
      <section className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            Out of Stocks ({outOfStockCount})
          </button>
        </div>

        {/* Right Toggle: Auto Notification Switch */}
        <div className="flex items-center gap-2.5">
                ...
                {products.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="grid size-8 shrink-0 place-items-center rounded-xl bg-orange-50 text-[#FF9F43] font-black text-xs overflow-hidden border border-orange-200/40 shadow-2xs">
                            {item.image ? (
                              <img
                                src={item.image}
                                alt=""
      ...
      <Modal
        ...
        title={
          stockAction === "add"
            ? `Restock Product — ${stockProduct?.name || ""}`
            : `Reconcile Stock — ${stockProduct?.name || ""}`
        }
        description="Update in-stock quantity and save the audit movement in ledger."
      ...
      >
        {stockProduct && (
          <StockActionForm
            ...
            values={stockValues}
            onActionChange={(e) => setStockAction(e.target.value)}
